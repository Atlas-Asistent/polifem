...
3. **Archive** (`polifem_archive.py`) — Move old entries (7+ days) to Obsidian or Markdown files
...
# Archive old entries (7+ days) to Markdown
...
| `ARCHIVE_AGE_DAYS` | 7 | Days before entries are archived |
...
### 2026-09-02
- **Retention aligned to 7 days** — `ARCHIVE_AGE_DAYS` changed from 3 to 7 to match docs.
- **toolResult truncation** — `toolResult` content truncated to 300 chars in buffer (full content kept in `fullContent` field when longer).
- **Tags & links** — entries now include `tags` (hashtags) and `links` (Obsidian wiki-links) extracted from content; buffer version bumped to 2.
- **Search on tags/links** — `polifem search` now matches tags and links in addition to content.
- **Archive output** — archived markdown now includes tags and links.